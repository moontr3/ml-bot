...
import io
from .handlers.base import Messages
from .handlers import command_list
...
async def crosspost_message(messages: List[aiogram.types.Message]):
    bot = messages[0].bot
...
async def handle_commands(messages: Messages):
    out = []

    for callable in command_list:
        out_messages = await callable(messages)
        if out_messages:
            out.extend(out_messages)
...
async def on_media_group(messages: List[aiogram.types.Message]):
    await crosspost_message(messages)
...
async def on_message(message: aiogram.types.Message):
    ...
    pack = Messages([message], dcbot, manager)
    out: List[List[aiogram.types.Message]] = await handle_commands(pack)

    for i in out:
        await crosspost_message(i)
...
async def on_media_group(messages: List[aiogram.types.Message]):
    ...
    pack = Messages(messages, dcbot, manager)
    out: List[List[aiogram.types.Message]] = await handle_commands(pack)
